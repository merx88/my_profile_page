    ...
    <div className="flex w-[760px] h-auto p-[36px_40px] flex-col items-start gap-[10px] shrink-0 rounded-[20px] bg-gradient-to-t from-[rgba(0,0,0,0.10)] to-[rgba(255,255,255,0.10)] shadow-[0_8px_24px_rgba(0,0,0,0.15)]">
      <div className="flex w-[679px] flex-col items-start gap-[16px]">
        ...
        <div className="text-[#454545] font-inter text-[16px] font-medium leading-normal">
          ...
          <br />
          <br />
          🛣️ 계획 : 이더리움은 현재 확장성, 보안성, 2.0 향해 나아가고 있는 중임,
          merge를 통해 PoS를 도입했지만 아직 속도 개선을 완벽하게 하지는 못했고
          비탈릭이 레이어2 솔루션을 적극적으로 활용할 것이라 밝힘 최근
          업그레이드에서도 레이어2를 위한 블롭을 도입함 <br />
          <br />
        ...
        <div className="text-[#454545] font-inter text-[16px] font-medium leading-normal">
          TVL : 에치된 코인 수 = (투자자)말그대로 예치된 코인의 수 투자자들은
          예치된 코인이 많을 수록 코인가치가 올라가기 때문에 중요함
          <br />
          <br />
          Daily TPS : 트랜잭션 속도 = (개발자) 앱을 만드는 개발자들은 속도가
          빨라야 더 나은 ux를 제공할수있기에 중요함
          <br />
          <br />
          30D tx count : 사용량 = 당연히 사용량은 해당 레이어2의 활성화 정도를
          보는거기 때문에 누구에게나 중요함
          <br />
          <br />
          DATA AVAILABILITY : 데이터 가용성 = 검증을 최대한 할수있어야 하기
          때문에 데이터를 접근 하는 정도는 누구에게나 중요함
          <br />
          <br />
          STATE VALIDATION : 상태 증명 = 블록체인에 악의적인 사용자를 걸러내고
          잘 저장하기 위해 상태를 검증하는 것은 중요함
          <br />
          <br />
          SEQUENCER fAILURE : 시퀀서 실패 = 레이어2는 트랜잭션을 모아 보내기
          때문에 이는 실패지점이 될수있기에 중요함
          <br />
          <br />
          PROPOSER fALURE : 제안자 실패 = (밸리데이터) 자유로운 검증자 참여는
          필수이기에 중요함
          <br />
          <br />
          이와 같은 지표들로 레이어 2 솔루션을 비교 가능하고 사용자가 직접
          비교하며 자신에게 맞는 레이어 2 솔루션을 고를 수있도록 지원하는
          시스템을 만들고자 함
        </div>
      </div>
    </div>
